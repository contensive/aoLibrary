...
Imports Contensive.Addons.ResourceLibrary.Models.Db
Imports Contensive.Models.Db
Imports Contensive.Addons.ResourceLibrary.Controllers
Imports Contensive.Addons.ResourceLibrary.Contensive.Addons.ResourceLibrary
Imports Contensive.Addons.ResourceLibrary.Models.Domain

Namespace Views
    '
    Public Class LegacyLibraryClass
        '
        Public iconFiles As New List(Of FileTypeModel)
        '
        Public folders() As FolderTypeModel = Array.Empty(Of FolderTypeModel)
        Public folderCnt As Integer
        Public FolderIdIndex As New Controllers.FastIndexClass
        Public FolderNameIndex As New Controllers.FastIndexClass
        Public FolderPathIndex As New Controllers.FastIndexClass
        '
        ...
        '=================================================================================
        '
        Public Function getResourceLibrary(cp As CPBaseClass) As String
            Try
                Using ae As New applicationController(cp)
                    '
        ...
        '=================================================================================
        '
        Private Function GetForm(cp As CPBaseClass, ae As applicationController) As String
            Dim result As String = ""
            Try
                ...
                If Not (False) Then
                    ...
                    Dim AllowThumbnails As Boolean = cp.User.GetBoolean("LibraryAllowthumbnails", False)
                    ...
                    Dim topFolderID As Integer = LoadFolders_returnTopFolderId(cp, ae.topFolderPath)
                    '
                                ...
                                For Each baseNode In .ChildNodes
                                    ...
                                    Select Case LCase(baseNode.Name)
                                        Case "filetype"
                                            hint = "070"
                                            Ptr = Ptr + 1
                                            Dim newFileType As New FileTypeModel
                    ...
                    If currentFolderID = 0 Then
                        ...
                        Call cp.User.SetProperty("LibraryFolderID", currentFolderID.ToString())
                    End If
                    Dim folder As Models.Db.LibraryFolderModel = DbBaseModel.create(Of Models.Db.LibraryFolderModel)(cp, currentFolderID)
                    ...
                    Dim AllowPlaceColumn As Boolean = ae.allowPlace And ((SourceMode = SourceModeFromLinkDialog) Or (SourceMode = SourceModeFromDownloadRequest))
                                ...
                                If AllowLocalFileAdd Then
                                    ...
                                    For UploadPointer = 1 To AddFolderCount
                                        ...
                                        If folderName <> "" Then
                                            If IsContentManagerFolders And (Not cp.User.IsAdmin) And (currentFolderID = 0) Then
                                                ...
                                                Exit For
                                            Else
                                                Dim libraryFolder As Models.Db.LibraryFolderModel = DbBaseModel.addDefault(Of Models.Db.LibraryFolderModel)(cp)
                                    ...
                                    For UploadPointer = 1 To UploadCount
                                        ...
                                        If uploadFilename <> "" Then
                                            hint = "410"
                                            Dim libraryFile As LibraryFileModel = DbBaseModel.addDefault(Of LibraryFileModel)(cp)
                                            ...
                                            FilenameNoExtension = ""
                                            AltSizeList = ""
                                            Pos = InStrRev(uploadFilename, ".")
                                            If Pos > 0 Then
                                                FileExtension = Mid(uploadFilename, Pos + 1)
                    ...
                    End If
                    '
                    Dim parentFolder As Models.Db.LibraryFolderModel = Nothing
                    ...
                    If currentFolderID <> 0 Then
                        ...
                        '
                        folder = DbBaseModel.create(Of Models.Db.LibraryFolderModel)(cp, currentFolderID)
                        If (folder IsNot Nothing) Then
                            parentFolderID = folder.parentId
                        End If
                        parentFolder = DbBaseModel.create(Of Models.Db.LibraryFolderModel)(cp, parentFolderID)
                        Criteria = "(ParentID=" & KmaEncodeSQLNumber(cp, currentFolderID) & ")"
                    ElseIf ae.topFolderPath <> "" Then
                        '
                        ' ----- Rootfolder given, lookup record and get ParentID
                        '
                        folder = DbBaseModel.createByUniqueName(Of Models.Db.LibraryFolderModel)(cp, ae.topFolderPath)
                        If (folder IsNot Nothing) Then
                            parentFolderID = 0
                    ...
                    If True Then
                        ...
                        Dim folderList As List(Of Models.Db.LibraryFolderModel) = DbBaseModel.createList(Of Models.Db.LibraryFolderModel)(cp, Criteria, SortField)
                        ...
                        For Each folder In folderList
                            ...
                            FormDetails &= GetFormRow_ChildFolders(cp, IconFolderClosed, IconLink, "", ChildFolderName, "", cp.Utils.EncodeDate(ModifiedDate), RowCount, EditLink, folder.description, "CHILD", "", "", "", "", "", 0, folder.id, AllowEditColumn, AllowPlaceColumn, AllowSelectColumn)
                            RowCount = RowCount + 1
                        Next
                        ...
                        Else
                            Criteria = "(FolderID=" & KmaEncodeSQLNumber(cp, currentFolderID) & ")"
                        End If
                        Dim fileList As List(Of LibraryFileModel) = LibraryFileModel.createList(Of LibraryFileModel)(cp, Criteria, SortField)
                        For Each file In fileList
                            ...
                            Dim ImageWidthText As String = file.width.ToString()
                            Dim ImageHeightText As String = file.height.ToString()
                            If ModifiedDate <= Date.MinValue Then
                            ...
                            ' -- get file size
                            If fileSize = 0 Then
                                Dim Pathname As String = cp.CdnFiles.PhysicalFilePath & Replace(Filename, "/", "\")
                    ...
                    For RowCount = RowCount To iMinRows
                        FormDetails &= GetFormRow_Blank(cp, "", "", "", "", "", Nothing, RowCount, "", "", "BLANK", "", "", "", "", "", 0, currentFolderID, AllowEditColumn, AllowPlaceColumn, AllowSelectColumn, ColumnCnt)
                    Next
                    '
                    ' Upload link
                    '
                    ...
                    '
                    Dim FormFolders As String = GetRLNav(cp, currentFolderID, ae.topFolderPath, topFolderID)
        ...
        Private Function GetFormRow_Folders(cp As CPBaseClass, ignore0 As String, IconLink As String, IconOnClick As String, Name As String, NameLink As String, ModifiedDate As Date, RowCount As Integer, EditLink As String, Description As String, FileType As String, ResourceName As String, ResourceLink As String, ImageAlt As String, ImageWidth As String, ImageHeight As String, RecordID As Integer, FolderID As Integer, AllowEditColumn As Boolean, AllowPlaceColumn As Boolean) As String
            ...
            Try
                ...
                Dim CellStart As String = "<td class=""left  " & RowClass & """>"
                Dim CellStartRight As String = "<td class=""right  " & RowClass & """>"
                Dim CellStart2 As String = "<td class=""left  " & RowClass & """>"
                Dim CellStart5 As String = "<td class=""left  " & RowClass & """>"
        ...
        Private Function GetFormRow_ChildFolders(cp As CPBaseClass, ignore0 As String, IconLink As String, IconOnClick As String, Name As String, NameLink As String, ModifiedDate As Date, RowCount As Integer, EditLink As String, Description As String, FileType As String, ResourceName As String, ResourceLink As String, ImageAlt As String, ImageWidth As String, ImageHeight As String, RecordID As Integer, FolderID As Integer, AllowEditColumn As Boolean, AllowPlaceColumn As Boolean, AllowSelectColumn As Boolean) As String
            ...
            Try
                ...
                End If
                '
                Dim CellStart As String = vbCrLf & "<td class=""left "">"
                Dim CellStartCenter As String = vbCrLf & "<td class=""center "">"
                Dim CellStartRight As String = vbCrLf & "<td class=""right "">"
                Dim CellEnd As String = "</td>"
                Dim DateString As String
                '
                If ModifiedDate <= Date.MinValue Then
        ...
        Private Function GetFormRow_Files(cp As CPBaseClass, fileSize As Integer, IconLink As String, IconOnClick As String, Name As String, NameLink As String, ModifiedDate As Date, RowCount As Integer, EditLink As String, Description As String, FilenameExt As String, ResourceName As String, ResourceLink As String, ImageAlt As String, ImageWidth As String, ImageHeight As String, RecordID As Integer, FolderID As Integer, AllowThumbnails As Boolean, FileTypeFilter As String, ThumbNailSrc As String, SourceMode As Integer, AllowEditColumn As Boolean, AllowPlaceColumn As Boolean, AllowSelectColumn As Boolean) As String
            ...
            Try
                ...
                Dim CellStartCenter As String = vbCrLf & "<td class=""center "">"
                Dim CellStartRight As String = vbCrLf & "<td class=""right "">"
                Dim CellStartRightPad As String = vbCrLf & "<td class=""right  pr-3"">"
                Dim CellEnd As String = "</td>"
                Dim DateString As String
                '
                If ModifiedDate <= Date.MinValue Then
                ...
                Else
                    ...
                    For Each iconFile As FileTypeModel In iconFiles
                ...
                End If
                '
                GetFormRow_Files = GetFormRow_Files & CellStartRightPad & DateString & CellEnd
        ...
        Private Function GetFormRow_Options(cp As CPBaseClass, FolderID As Integer, topFolderPath As String, ColumnCnt As Integer, IsContentManagerFiles As Boolean, IsContentManagerFolders As Boolean, hasModifyAccess As Boolean) As String
            ...
            Try
                ...
                If hasModifyAccess Then
                    ...
                    & "<td class=""left"" align=""left"" colspan=2>" & kmaAddSpan("Add Folder&nbsp;", "") & "</td>" _
                    & "<td class=""left"" Width=""99%"" align=""left"">" & kmaAddSpan("Description&nbsp;", "") & "</td>" _
                    & "</tr><tr>" _
                ...
                If hasModifyAccess Then
                    ...
                    & "<tr>"
                    FileCell = FileCell _
                    & "<td class=""left"" align=""left"" colspan=2>" & kmaAddSpan("Add Files&nbsp;", "") & "</td>" _
                    & "<td class=""left"" Width=""100"" align=""left"">" & kmaAddSpan("Name&nbsp;", "") & "</td>" _
                    & "<td class=""left"" Width=""100"" align=""left"">" & kmaAddSpan("Description&nbsp;", "") & "</td>" _
                ...
                If GetFormRow_Options <> "" Then
                    ...
                    GetFormRow_Options = "<tr><td class=""bg-light pt-3 left"" colspan=" & (ColumnCnt) & ">" & GetFormRow_Options & "</td></tr>"
                End If
                '
        ...
        Private Function IsInFolder(cp As CPBaseClass, topFolderID As Integer, FolderID As Integer, Optional ParentPath As String = "") As Boolean
            Try
                ...
                Dim folder = DbBaseModel.create(Of Models.Db.LibraryFolderModel)(cp, FolderID)
        ...
        Private Function GetParentFoldersLink(cp As CPBaseClass, topFolderPath As String, topFolderID As Integer, currentFolderID As Integer, FolderID As Integer, RefreshQS As String, ChildIDList As String) As String
            ...
            Try
                ...
                If (FolderID = 0) Or (FolderID = topFolderID) Then
                    ...
                    End If
                Else
                    Dim LibraryFolder As Models.Db.LibraryFolderModel = DbBaseModel.create(Of Models.Db.LibraryFolderModel)(cp, "ID=" & FolderID)
                    Dim ParentID As Integer
        ...
        Private Function AllowFolderAccess(cp As CPBaseClass, FolderID As Integer, ParentID As Integer) As Boolean
            ...
            '            Dim LibraryFolder As Models.Db.LibraryFolderModel = DbBaseModel.create(Of Models.Db.LibraryFolderModel)(cp, ParentID)
            '            If Not (LibraryFolder Is Nothing) Then
        ...
        Private Function LoadFolders_returnTopFolderId(cp As CPBaseClass, topFolderPath As String) As Integer
            ...
            Try
                ...
                Dim foldersList As List(Of Models.Db.LibraryFolderModel) = DbBaseModel.createList(Of Models.Db.LibraryFolderModel)(cp, "")
                folderCnt = 0
